...
This tutorial is meant to be a companion to the "KEGG Pathway Enrichment" protocol in benchling. The purpose of this is to provide an example of running a over-representation test using a gene list to find over-represented pathways. Here we are going to demonstrate how to do this if there is no bioconductor annotation for the species (http://bioconductor.org/packages/release/BiocViews.html#___OrgDb) you are analyzing. This assumes that you have already completed a mapping of the genes in your organism to KEGG IDs as detailed in the "Mapping genes to KEGG IDs" section in the "KEGG Pathway Enrichment" protocol. If your organism is in the database, then you can easily map the genes to required KEGG IDs using the bitr() function (see manual for more info). Online reference: https://bioconductor.org/packages/release/bioc/vignettes/clusterProfiler/inst/doc/clusterProfiler.html

For this example will use Xenopus laevis as our species and the Xenopus laevis Notch RNA-Seq experiment GFP vs ICD stage 18 comparison. Here GFP is the control condition and ICD is the experimental. Note that there is a bioconductor annotation for Xenopus laevis, however, the gene names that are in the DESeq2 output do not map to the annotation, so we will perform the analysis as if we did not have an annotation. 
...
## Make sure Xenopus laevis is in the dataase
We can search our organism by either using its abbreviation (found on KEGG: http://www.genome.jp/kegg-bin/show_organism?org=xla) or by the scientific name. We need to do this because we will be converting the gene names to KEGG IDs specific to Xenopus laevis. 
```{r}
# by symbol
search_kegg_organism('xla', by='kegg_code')

# by scientific name
search_kegg_organism('Xenopus laevis', by='scientific_name')
```

## Load Gene-KEGG ID csv
Since we do not have a bioconductor annotation, we need to create Here we are loading the csv that we made with the gene list and corresponding KEGG IDs. This list should contain all of the genes expressed in the experiment output by DESEq2. 
```{r}
# read in csv

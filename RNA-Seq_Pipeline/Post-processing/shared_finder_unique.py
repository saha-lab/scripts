...
"""
Takes in the results of two csv files output from the
unique_finder_two.py script and finds the shared genes
between these two lists

USAGE: shared_finder_unique.py <unique_csv_1> <unique_csv_2> <output_dir>
OUTPUT: output_dir_unique_csv_1.csv_shared_results.csv
...
def shared_finder(file1reader, file2reader):
	...
	list1 = []
	list1 = file1reader[file1reader.columns[0]] # modified for using OG pairwise comparisons 
	...
	for item in tqdm(list_store): # iterating through list_store to get the lfc and padj
		...
		if item == genelist1:
			lfc_store.append(file1reader.iloc[gene_index, 3]) 
			padj_store.append(file1reader.iloc[gene_index, 7])
			#parent_rank.append(file1reader.iloc[gene_index, 2])
			parent_rank.append(gene_index) # modified for comparing pairiwse comparison results to shared list results
			name.append(file1reader.iloc[gene_index, 1])
		else:
			lfc_store.append(file2reader.iloc[gene_index, 3]) 
			padj_store.append(file2reader.iloc[gene_index, 4])
			parent_rank.append(file2reader.iloc[gene_index, 2])
			name.append(file2reader.iloc[gene_index, 1])
		count += 1 
	...
	list_store = numpy.array(list_store)
	parent_rank = numpy.array(parent_rank)
	name = numpy.array(name) 
	shared = numpy.array(shared) 

	# dictionary building pandas data frame
	dataframe = pandas.DataFrame({'shared_genes' : shared, 'name' : name, 'parent_list' : list_store, 'parent_rank' : parent_rank, 'log2FoldChange' : lfc, 'padj' : padj})
	# re-ordering columns to reflect above (default is to arrange alphabetical order)
	cols = dataframe.columns.tolist()
	cols = ['shared_genes', 'name', 'parent_list', 'parent_rank', 'log2FoldChange', 'padj']
